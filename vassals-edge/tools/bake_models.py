...
Run:  python3 tools/bake_models.py                # bake from the spec (no Blender)
      python3 tools/bake_models.py --merge-only   # keep models/models_v2.json as exported by Blender, only regenerate models.js
"""
import base64
import json
import math
...
def main():
    ...
    v2_path = os.path.join(ROOT, 'models', 'models_v2.json')
    if '--merge-only' in sys.argv:
        with open(v2_path) as f:
            v2 = json.load(f)
        tris = sum(len(base64.b64decode(p['i'])) // 6 for parts in v2.values() for p in parts.values())
        source = 'models/models_v2.json (Blender export)'
    else:
        v2, tris = bake_all(MODELS)
        with open(v2_path, 'w') as f:
            json.dump(v2, f, separators=(',', ':'))
        source = 'tools/models_spec.py (Python baker)'
    ...
    js = ("/* Generated by tools/bake_models.py — do not edit by hand.\n"
          "   v1: 14 models authored in Blender (see docs/ORIGINAL_HANDOFF.md). v2: %d models, source: %s.\n"
          "   Format: MODEL_DATA[model][part] = { a: socket, c: colour key, v: base64 int16 mm xyz, i: base64 uint16 indices }. */\n"
          "export const MODEL_DATA = %s;\n") % (len(v2), source, json.dumps(merged, separators=(',', ':')))
