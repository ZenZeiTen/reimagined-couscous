...
import { Camera, Raycaster, WallRenderer, SpriteRenderer, Shading, Texture, TextureRegistry, SpriteSheet, SpriteSheetRegistry, packRGBA, unpackR, unpackG, unpackB, unpackA, shadePixel, brickTexture, humanoidSheet, type Billboard } from '../src/renderer';
...
describe('SpriteSheet metadata', () => {
  ...
  it('procedural sheets satisfy the same schema', () => {
    const grunt = humanoidSheet({ name: 'grunt', directions: 8, frameSize: 32 });
    expect(grunt.directions).toBe(8);
    expect(grunt.animationNames()).toEqual(['idle', 'walk', 'attack', 'hurt', 'die']);
    expect(grunt.texture.width).toBe(8 * 32);
    // Some opaque pixels must exist in each direction of the idle frame.
    for (let d = 0; d < 8; d++) {
      const idx = grunt.frameIndex('idle', 0, d);
      let opaque = 0;
      for (let y = 0; y < 32; y++) for (let x = 0; x < 32; x++) if (unpackA(grunt.texture.pixels[(grunt.rectY[idx]! + y) * grunt.texture.width + grunt.rectX[idx]! + x]!) > 0) opaque++;
